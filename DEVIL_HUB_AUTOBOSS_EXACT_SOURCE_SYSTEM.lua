...
do
    ...
    if blacklist[LocalPlayer.Name] then
        task.spawn(function()
            ...
            -- إذا كانت بيانات اللاعب غير جاهزة، لا ننتظر؛ يتم طرده مباشرة.
            ...
            -- نفّذ الـRebirth فقط إذا كانت القوة تسمح به.
            -- إذا لم تكن القوة كافية، لا ندخل في حلقة انتظار أو تدريب.
            ...
            -- في جميع الحالات يتم طرد الحساب المحظور بسرعة.
            task.wait(0.2)
...
do
    ...
    --==================================================
    -- ESCANOR AUTO BOSS CORE
    -- Directly ported from gistfile1 (6).txt.
    -- Only the DEVIL HUB UI wrapper is used instead of the
    -- standalone ScreenGui from the source script.
    --==================================================
    ...
    BossFight:AddLabel("AUTO BOSS + AUTO PUNCH + AUTO CLAIM CHEST + SMOOTH DODGE")

    local function CreateToggle(name, key, defaultState, callback)
        local state = defaultState
        local switch
        switch = BossFight:AddSwitch(name, function(newState)
            state = newState
            callback(state)
        end)
        ToggleSetters[key] = function(newState)
            state = newState
            pcall(function() switch:Set(newState) end)
            callback(state)
        end
    end

    CreateToggle("AUTO BOSS COMBAT", "AutoBoss", AutoBoss, function(s)
        AutoBoss = s
    end)
    CreateToggle("AUTO PUNCH", "AutoPunch", AutoPunch, function(s)
        AutoPunch = s
    end)
    CreateToggle("AUTO CLAIM CHEST", "AutoChest", AutoChest, function(s)
        AutoChest = s
    end)

    BossFight:AddSlider("MELEE DISTANCE", function(v)
        AttackDistance = v
    end, {min = 1, max = 15})
    BossFight:AddSlider("STOMP DODGE DISTANCE", function(v)
        DodgeDistance = v
    end, {min = 50, max = 300})

    -- EXACT SetAllToggles behavior from the source Auto Boss script.
    ...
    local function GetHealth(Model)
        ...
        for Name, Value in pairs(Model:GetAttributes()) do
            ...
            if typeof(Value) == "number" then if N:find("hp") or N:find("health") then if N:find("max") then MaxHP = Value else CurrentHP = Value end end end
        end
    ...
    local hpLabel = BossFight:AddLabel("❤️ SEARCHING BOSS...")

    task.spawn(function()
        while BossFight do
            local Arena = GetArena()
            if not Arena then
                pcall(function() hpLabel:Set("❤️ BOSS ARENA NOT FOUND") end)
            else
                local Boss = FindBoss()
                if not Boss then
                    pcall(function() hpLabel:Set("❤️ SEARCHING BOSS...") end)
                else
                    local HP, MaxHP = GetHealth(Boss)
                    if HP then
                        if HP <= 0 then SetAllToggles(false) end
                        if MaxHP then
                            pcall(function() hpLabel:Set("👑 " .. Boss.Name .. "\n❤️ " .. FormatNumber(HP) .. " / " .. FormatNumber(MaxHP)) end)
                        else
                            pcall(function() hpLabel:Set("👑 " .. Boss.Name .. "\n❤️ " .. FormatNumber(HP)) end)
                        end
                    else
                        pcall(function() hpLabel:Set("❤️ BOSS HP NOT FOUND") end)
                    end
                end
            end
            task.wait(0.5)
    ...
    -- EXACT Auto Chest core from gistfile1 (6).txt
    ...
    -- EXACT Auto Punch core from gistfile1 (6).txt
    task.spawn(function()
    ...
    -- EXACT Smooth Dodge core from gistfile1 (6).txt
    ...
    -- EXACT 3-hour timer behavior from gistfile1 (6).txt
    ...
    RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            timerLabel:Set(string.format("⏳ %02d:%02d:%02d", displayHours, displayMinutes, displaySeconds))
        end)
        ...
        if secondsLeft == 0 then
            pcall(function() timerLabel:Set("🚨 BOSS SPAWNING NOW!") end)
